...
def request_retry(type_of_request, url, max_retry, body=None, headers=None,
                  auth=None, params=None, proxy=None, ssl=False):
    """
    ...
    :param params: Query strings for the request
    ...
    count = 0
    response = None
    valid_response_codes = [requests.codes.ok, requests.codes.created, requests.codes.accepted]
    logging.debug(f"type_of_request: {str(type_of_request)}")
    logging.debug(f"url: {str(url)}")
    while count < max_retry:
        try:
            response = make_request_based_on_input(type_of_request, url, body, headers, auth,
                                                   params, proxy, ssl)
...
def make_request_based_on_input(request_type, url, body, headers, auth, params, proxy, ssl):
    """
...
def convert_response_to_json(response):
    ...
    try:
        ...
        raise Exception('Got invalid response from request and could not convert it to JSON!') from json_error
    return json_response
