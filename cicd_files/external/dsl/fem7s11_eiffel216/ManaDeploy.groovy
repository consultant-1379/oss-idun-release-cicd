def pipelineBeingGeneratedName = "oss-idun-release-cicd_MANA_Deploy"
...
<div style="width:fit-content;background:#fbf6e1;padding:1em;border-radius:1em;box-shadow: 0 0.1em 0.3em #525000">
    ...
    <a href="mailto:PDLENMCOUN@pdl.internal.ericsson.com?Subject=oss-idun-release-cicd_MANA_Deploy%20Job">&#128231; Send Mail to provide feedback</a></p>
</div>
"""
    )

    parameters {
        stringParam("SLAVE_LABEL", "DUMMY", "Label of the Jenkins slave where this jenkins job should be executed.") // Update the GE name
        ...
        stringParam("CSAR_DOWNLOAD_LOCATION_PATH", "", "Path on server where the working directory is available. E.g. /mnt. The working directory 'swgw_download_dir' will automatically be created.")
    }
    ...
    stages {
        ...
        stage("Unpack and init Deployment Manager") {
            steps {
                ...
                sh "sudo docker load --input ${env.DEPLOYMENT_MANAGER_IMAGE}.tar"
                sh "sudo docker run -u \\$(id -u ${USER}):\\$(id -g ${USER}) -v /var/run/docker.sock:/var/run/docker.sock -v ${env.WORKSPACE}:/workdir -v /etc/hosts:/etc/hosts ${env.DEPLOYMENT_MANAGER_IMAGE}:${params.OSS_DEPLOYMENT_MANAGER_VERSION} init"
            }
        }
                ...
                failure {
                    script {
                        withCredentials([file(credentialsId: env.KUBECONFIG_FILE, variable: "KUBECONFIG")]) {
                            ...
                            sh "sudo docker run --network=host -u \\$(id -u ${USER}):\\$(id -g ${USER}) --init --rm -v /var/run/docker.sock:/var/run/docker.sock -v ${env.WORKSPACE}:/workdir -v /etc/hosts:/etc/hosts --workdir ${env.WORKSPACE} ${env.DEPLOYMENT_MANAGER_IMAGE}:${params.OSS_DEPLOYMENT_MANAGER_VERSION} collect-logs --namespace ${params.NAMESPACE} || true"
