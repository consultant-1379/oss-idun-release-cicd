def pipelineBeingGeneratedName = "oss-idun-release-cicd_Unpack_And_Push_Images"
...
<div style="width:fit-content;background:#fbf6e1;padding:1em;border-radius:1em;box-shadow: 0 0.1em 0.3em #525000">
    ...
    <a href="mailto:PDLENMCOUN@pdl.internal.ericsson.com?Subject=oss-idun-release-cicd_Unpack_And_Push_Images%20Job">&#128231; Send Mail to provide feedback</a></p>
</div>
"""
    )
...
pipeline {
    ...
    stages {
        ...
        stage("Unpack and Push Docker Images to Registry") {
            steps {
                sh "sudo docker run -u 0:0 -v /var/run/docker.sock:/var/run/docker.sock -v ${env.WORKSPACE}:/workdir -v /etc/hosts:/etc/hosts ${env.DEPLOYMENT_MANAGER_IMAGE}:${params.OSS_DEPLOYMENT_MANAGER_VERSION} image push --docker-timeout 1200"
            }
        }
    }
    post {
